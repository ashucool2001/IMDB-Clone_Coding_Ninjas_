...
async function addToFavourites(event) {
    ...
    if (movie) {
        ...
        if (!favouritesList.some(m => m.imdbID === movie.imdbID)) {
            ...
            alert(`${movie.Title} added to your favourites!`);
        } else {
    ...
    results.forEach(movie => {
        ...
        movieCard.innerHTML = `
            ...
            <div class="card-body">
                ...
                <button class="btn btn-primary btn-sm favourite-button" data-imdbid="${movie.imdbID}">Add Favourites</button>
                <a href="movie.html?id=${movie.imdbID}" class="btn btn-secondary btn-sm more-button">Movie Details</a>
            </div>
        `;
